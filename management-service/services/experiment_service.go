...
type ExperimentService interface {
	...
	ValidatePairwiseExperimentOrthogonality(projectId int64, listExpParams ListExperimentsParams, segmenters []string) error
...
func (svc *experimentService) CreateExperiment(
	...
	err = svc.services.SegmenterService.ValidateExperimentSegment(settings.Config.Segmenters.Names, expData.Segment)
	if err != nil {
		return nil, errors.Newf(errors.BadInput, err.Error())
	}
	...
	segmenterTypes := svc.services.SegmenterService.GetSegmenterTypes()
...
func (svc *experimentService) EnableExperiment(settings models.Settings, experimentId int64) error {
	...
	err = svc.validateExperimentOrthogonalityInDuration(&experimentId, settings,
		experiment.Segment.ToRawSchema(segmenterTypes), experiment.Tier, experiment.StartTime, experiment.EndTime)
	if err != nil {
		return err
	}
...
func (svc *experimentService) DisableExperiment(projectId int64, experimentId int64) error {
	...
	// Convert to the format expected by the Message Queue
	protoExpResponse, err := expDBRecord.ToProtoSchema(svc.services.SegmenterService.GetSegmenterTypes())
	if err != nil {
		return err
	}
	err = svc.services.PubSubPublisherService.PublishExperimentMessage("update", protoExpResponse)
...
func (svc *experimentService) validateExperimentOrthogonality(
	...
	if len(filteredExps) > 0 {
		err = svc.services.SegmenterService.ValidateSegmentOrthogonality(segmenters, segment, filteredExps)
		if err != nil {
...
func (svc *experimentService) validateExperimentOrthogonalityInDuration(
	...
	return svc.validateExperimentOrthogonality(experimentId, segment, exps, settings.Config.Segmenters.Names)
}

func (svc *experimentService) ValidatePairwiseExperimentOrthogonality(
	projectId int64,
	listExpParams ListExperimentsParams,
	segmenters []string,
) error {
	id := models.ID(projectId)
	exps, err := svc.listAllExperiments(id, listExpParams)
	if err != nil {
		return err
	}
	...
	for i := 0; i < len(exps)-1; i++ {
		currExp := exps[i]
		otherExps := exps[i+1:] // Take all the remaining elements
		...
		err = svc.validateExperimentOrthogonality(
			&experimentId,
			currExp.Segment.ToRawSchema(svc.services.SegmenterService.GetSegmenterTypes()),
			otherExpsByTier,
			segmenters,
		)
		if err != nil {
